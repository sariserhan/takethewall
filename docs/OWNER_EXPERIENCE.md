...
## Resume an unpaid checkout

Embedded checkout offers **Email me a resume link**. This is requested by the visitor, not an automatic abandoned-cart campaign. It sends one branded message per checkout through the existing email queue, from `account@takethewall.com` with support replies. Test-mode subjects start with `[TEST]`. The existing `/owner` recovery form also finds unpaid checkouts by their checkout email and requests a resume link after checking Stripe; completed payments still receive the private owner-dashboard link.

The link uses a private fragment (`/#resume=…`), removed from the address bar after loading. Its capability hash and independent seed are stored on the purchase, using the existing `CLAIM_TOKEN_SECRET`. The server checks the purchase, environment, moderation status, and link validity, then retrieves the original Stripe session. An open unpaid session returns its existing embedded checkout; no new purchase or takeover is created. Original content stays attached to that checkout. Paid sessions use verified recovery and confirmation; asynchronous payments still processing do not offer a second payment. Expired sessions offer a new takeover, not a silently recreated charge. Starting a new takeover clears the old browser draft and requires entering content again.

Requests cannot choose a different recipient. Backend and IP rate limits, email-policy suppression, and one-job-per-checkout deduplication apply. Delayed delivery is skipped if the app has recorded payment or the checkout has expired. The same link can be reused during its validity period; email link scanners do not start payment, and the visitor must click Resume checkout before Stripe is checked. A browser may not preserve fragments in every email client; `/owner` recovery is the fallback.

No new environment variables. Deploy backend and frontend together. This change does not send messages to existing production visitors automatically. No live payment, production visitor email, or production deployment was performed during verification.

## Admin payment status and checkout timeline

Admin → Takeovers separates payment status from placement status/type. `kind: paid` is shown as “Checkout purchase,” not as evidence of payment. Labels include Awaiting payment, Payment processing, Paid, Expired, Paid — publication pending, and No payment required for issued/house placements. Existing refund/dispute flags remain visible. Processing comes from the last verified Stripe check; expiration is not inferred merely from a clock passing the checkout deadline.

Takeovers → Inspect → **Check Stripe status** retrieves the attached session after server and Convex administrator checks. It validates the environment, takeover references, amount, currency, and payment mode. Eligible pending paid sessions use the existing verified recovery path, which can publish and replace the current owner; the UI explains this before the action. Unpaid/processing/expired sessions never publish. Blocked or already-processed placements can be inspected without being republished. Recorded checks are audited with the authenticated administrator, session, environment, and result. Failed provider requests do not manufacture a status.

The on-demand timeline shows recorded draft creation, checkout attachment (new records), payment verification, publication, reign end, Stripe checks, and related email queue/provider events. Resend acceptance and delivered/bounced/complained events are distinct. A finished queue entry without provider evidence is not called a sent email. Older missing timestamps are described as unavailable. Reads are bounded to 50 recent related email jobs, 50 related audit records, and 10 provider events per email, using indexes; this is not an unlimited audit export. Checkout tokens and buyer emails are not part of the timeline response.

No production checks or reconciliations were performed while implementing this feature. Deploy the backend schema/functions and frontend together.

## Checkout safeguards and admin controls

- The preview shows the current owner. If ownership changes, the buyer must acknowledge the new owner before opening payment. The server also compares the reviewed owner with the current owner before creating the pending purchase. This is a review safeguard, not a reservation: ownership can change while Stripe is open.
- Admin → Settings → Checkout availability pauses new checkout requests. The homepage remains visible and its new-checkout button is disabled. Already-started checkouts may still complete and publish; pausing never blocks activation of an already-paid purchase. Resume uses the existing session, not a new checkout.
- Verified Stripe payment activation failures attempt to queue one admin notification per checkout, using the configured takeover-notification recipient and enabled setting. Live payments only. The email includes the takeover and Stripe references and links to `/admin`; use Takeovers → Inspect → Check Stripe status to reconcile. It records `Paid — publication pending`, not a completed publication. The original error still propagates so Stripe can retry. If Convex is unavailable, alert queuing can also fail; webhook retries and admin reconciliation remain necessary.
- Admin → Takeovers has payment status and Live/Test filters. Filters operate on batches of 50 records; Next examines older records, even if the current filtered batch is empty. Records without a payment environment only appear under All environments.
- No new environment variables or sender addresses are required. Publication alerts use the existing notifications sender. No production emails or payments are triggered by the automated tests.
