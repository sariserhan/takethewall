...
- VisitorPing's live API reports site activity and custom wall events. For the inspected retained window, it returned 38 `wall_impression` events with `traffic=all`, versus zero with `exclude_bots`. The app relays these events from Convex after its own visitor checks, so the provider classifies the relay server rather than the original browser. The custom-event reader now uses `traffic=all`, retaining exact event, takeover ID, and time filters. No provider numbers are added to the live Convex counters.
