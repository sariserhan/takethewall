...
## Micro-AMA, celebrations, and companion

Owners enable **Accept questions during this reign** in their protected owner dashboard. Questions are limited to 240 characters and are private until the owner publishes a reply (up to 600 characters). Visitors can submit at most three questions per ten minutes per server-derived IP hash; a wall accepts at most 30 per minute and 100 pending questions. The owner inbox loads the newest 50 pending questions and 20 answers. Dismiss questions or remove public replies from that dashboard. Replacement or moderation closes the public AMA immediately and rejects new submissions/replies. Public reporting uses the existing placement-report form; owners should never publish private information from a question.
