...
export function paperBrickSvg({
  ...
  const short = (value: string, max: number) => escape(value.length > max ? value.slice(0, max - 1) + "…" : value);
  const title = short(name, 40);
  const subtitle = short(detail.replace(/^https?:\/\//, "").replace(/\/$/, ""), 48);
  const faceTitle = short(name, 28);
  const faceDetail = short(detail.replace(/^https?:\/\//, "").replace(/\/$/, ""), 34);
 ...
 <g font-family="Arial,sans-serif" fill="#111"><text x="20" y="20" ${name.length > 24 ? 'textLength="165" lengthAdjust="spacingAndGlyphs"' : ""} font-size="8" font-weight="bold">${title}</text><text x="20" y="30" font-size="4.5">${subtitle}</text><text x="20" y="39" font-size="3">TAKE THE WALL · PAPER BRICK · ${number === null ? "HOUSE PLACEMENT" : "TAKEOVER #" + number}</text>
 ...
 ${art ? `<image href="${art}" x="49" y="97" width="42" height="13" preserveAspectRatio="xMidYMid meet"/>` : ""}
 <g font-family="Arial,sans-serif" fill="#111">
 <text x="70" y="${art ? 117 : 109}" text-anchor="middle" ${name.length > 12 ? 'textLength="54" lengthAdjust="spacingAndGlyphs"' : ""} font-size="${name.length > 18 ? 3.5 : 5}" font-weight="bold">${faceTitle}</text>
 <text x="70" y="${art ? 124 : 118}" text-anchor="middle" ${detail.length > 24 ? 'textLength="54" lengthAdjust="spacingAndGlyphs"' : ""} font-size="2.8">${faceDetail}</text>
 <text x="70" y="85" text-anchor="middle" font-size="4">${number === null ? "OWNER’S PAPER BRICK" : "TAKEOVER #" + number}</text>
 <text x="70" y="142" text-anchor="middle" font-size="2.4">TAKE THE WALL</text>
 <text x="133" y="108" text-anchor="middle" font-size="2.4">LIVE WALL</text>
 <text x="133" y="119" text-anchor="middle" font-size="2.2">Scan to visit</text></g>
