...
- Unit 2 complete: milestone/claim schema, atomic provisional detection, deterministic bounded cascade with future successors, versioned rules, OTP/session authentication, retryable secret-safe email, submission deadlines excluding review, public projections, and maintenance for reminders/trophy statistics. 76 tests, lint and typecheck passed before the next admin unit; local Convex deployment succeeded.
- Claim-link retry design: keyed 256-bit links derive from an independent random nonce and server-only CLAIM_TOKEN_SECRET. Database stores the nonce and token hash, never the raw link token; the separate secret allows reliable retries. OTP digests are keyed to resist offline six-digit guessing after a database-only leak.
