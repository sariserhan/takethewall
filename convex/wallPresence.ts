...
import { parseVisitorNumber } from "../lib/visitorping-webhook";
import { internalMutation, query } from "./_generated/server";
const presence = new Presence(components.presence);
const room = "wall";

// Persist enrichment even when the callback precedes the first heartbeat.
export async function setVisitorNumber(ctx: MutationCtx, visitorHash: string, value: number) {
  const visitorNumber = parseVisitorNumber(value);
  const row = await ctx.db.query("presenceLocations").withIndex("by_visitorHash", q => q.eq("visitorHash", visitorHash)).unique();
  if (row) {
    if (row.visitorNumber !== visitorNumber) await ctx.db.patch(row._id, { visitorNumber });
  } else await ctx.db.insert("presenceLocations", { visitorHash, city: "", country: "", visitorNumber });
}
...
export const live = query({
  args: {},
  returns: v.array(v.object({ id: v.string(), receivedAt: v.number(), visitorNumber: v.optional(v.number()), city: v.string(), country: v.string() })),
  handler: async ctx => {
    ...
    const rows = await Promise.all(online.map(async user => {
      ...
      return { ...(location.visitorNumber !== undefined ? { visitorNumber: location.visitorNumber } : {}), id: location._id, receivedAt: location._creationTime, city: location.city, country: location.country };
    }));
