...
export type CityDelta = { city: string; country: string; views: number; lastVisitAt?: number };
...
export function cityKey(city: string, country: string) { return `${country}:${cityName(city).toLowerCase()}`; }
export function addCityDelta(rows: CityDelta[], city: string, country: string, views: number, lastVisitAt?: number) {
  city = cityName(city);
  const row = rows.find(r => r.city.toLowerCase() === city.toLowerCase() && r.country === country);
  if (row) {
    row.views += views;
    if (lastVisitAt !== undefined) row.lastVisitAt = Math.max(row.lastVisitAt ?? 0, lastVisitAt);
  } else rows.push({ city, country, views, ...(lastVisitAt === undefined ? {} : { lastVisitAt }) });
}
...
export async function applyCityDelta(ctx: MutationCtx, date: string, delta: CityDelta) {
  ...
  const city = cityName(delta.city), key = cityKey(city, delta.country);
  const row = await ctx.db.query("dailyCityViews").withIndex("by_date_key", q => q.eq("date", date).eq("key", key)).unique();
  // Resolve from the ledger so corrected geography also removes stale recency.
  const latest = await ctx.db.query("visitLedger").withIndex("by_date_cityKey_occurredAt", q => q.eq("date", date).eq("cityKey", key)).order("desc").first();
  const recency = { lastVisitAt: latest?.occurredAt ?? 0 };
  if (row) await ctx.db.patch(row._id, { views: row.views + delta.views, ...recency });
  else await ctx.db.insert("dailyCityViews", { date, key, city, country: delta.country, views: delta.views, ...recency });
}
export async function readRadarCities(ctx: QueryCtx, date: string, views: number, uniqueVisitors: number) {
  ...
  if (recorded <= views) for (const row of live) addCityDelta(groups, row.city, row.country, row.views, row.lastVisitAt);
  ...
  return {
    ...
    cities: groups.filter(r => r.views > 0).sort((a, b) => (b.lastVisitAt ?? 0) - (a.lastVisitAt ?? 0) || a.city.localeCompare(b.city) || a.country.localeCompare(b.country)).map(row => ({ city: row.city, country: row.country, views: row.views, label: row.city ? `${row.city}, ${row.country}` : row.country !== "ZZ" ? `City not recorded, ${row.country}` : "Location not recorded" })),
  };
}
