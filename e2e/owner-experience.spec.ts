...
test("designer deletes selected blocks safely and keeps controls below canvas", async ({ page }, testInfo) => {
  const errors: string[] = [];
  page.on("pageerror", error => errors.push(error.message));
  await wallFixture(page);
  await page.addInitScript(() => sessionStorage.setItem("ttw-draft", JSON.stringify({ contentType: "personal", category: "personal", displayName: "My homepage", description: "Welcome", websiteUrl: "", logoUrl: "", uploadKey: "", buyerEmail: "owner@example.com", requestKey: crypto.randomUUID() })));
  await page.goto("/?take=1");
  await expect(page).toHaveTitle(/Take The Wall/i);
  ...
  const text = designer.getByRole("textbox", { name: "Block text", exact: true });
  await text.fill("Keep this block");
  ...
  await designer.getByRole("button", { name: "Add image", exact: true }).click();
  await designer.getByRole("button", { name: "Delete selected block", exact: true }).click();
  await expect(blocks).toHaveCount(initial);
  const canvasBox = await stage.boundingBox();
  const controlsBox = await designer.locator(".designer-inspector").boundingBox();
  expect(controlsBox!.y).toBeGreaterThanOrEqual(canvasBox!.y + canvasBox!.height);
  expect(Math.abs(controlsBox!.width - canvasBox!.width)).toBeLessThan(2);
  await designer.getByRole("button", { name: "Add heading", exact: true }).click();
  await designer.getByRole("textbox", { name: "Block text", exact: true }).fill("Your canvas. Your rules.");
