...
test("admin separates payment and placement status and checks a checkout timeline", async ({page}, info) => {
  ...
  await page.routeWebSocket(/convex.*\/sync/, socket => {
    ...
    socket.onMessage(raw=>{const msg=JSON.parse(String(raw));if(msg.type==="Authenticate")transition([],{identity:msg.baseVersion+1});if(msg.type==="ModifyQuerySet")transition(msg.modifications.map((q:{type:string,queryId:number,udfPath:string})=>q.type==="Remove"?{type:"QueryRemoved",queryId:q.queryId}:{type:"QueryUpdated",queryId:q.queryId,value:value(q.udfPath),logLines:[],journal:null}),{querySet:msg.newVersion});});
  });
  let checks=0;
